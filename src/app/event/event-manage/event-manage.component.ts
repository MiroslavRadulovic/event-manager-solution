...
export class EventManageComponent implements OnInit {
    ...
    const formElements = {
      ...
      address: [
        ...
        Validators.compose([
          ...
          /**
           * Min length validation for address (solution for task number 9).
           */
  ...
  private update(changes: Event): void {
    this.eventDataService.update(changes).subscribe(() => {
  ...
  private save(changes: Event): void {
    this.eventDataService.save(changes).subscribe((event) => {
      this.unsavedChanges = false;
